...
import { ADMIN_LOGIN_GQL, USER_LIST_QUERY } from "@/graphql/queries/main.quiries";
...
export default function SigninWithPassword() {
  ...
  const [adminLogin, { loading, error }] = useMutation(ADMIN_LOGIN_GQL);
  ...
  const onSubmit: SubmitHandler<Inputs> = async (formData) => {
    try {
      ...
      if (data && data.User_Admin_Login) {
        console.log("Login successful:", data);
        // Store the token in localStorage or a secure cookie
        localStorage.setItem('authToken', `Bearer ${data.User_Admin_Login.token}`);
