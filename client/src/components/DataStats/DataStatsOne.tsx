...
import { dataStats } from "@/types/templateTypes/dataStats";
import { useQuery } from "@apollo/client";
import { DASHBOARD_STATS } from "@/graphql/queries/main.quiries";
type DashboardStats = {
...
const DataStatsOne: React.FC<dataStats> = () => {
  const { loading, data, error, refetch } = useQuery(DASHBOARD_STATS, {
    variables: {},
  });
  const dataStatsList = [
    {
      ...
      color: "#3FD97F",
      title: "Total Rooms",
      value: data?.DashboardStats?.roomCount ?? 0,
      // growthRate: 0.43,
    },
    {
      icon: (
      ...
      value: `RS ${data?.DashboardStats?.paymentCount ?? 0}`,
      // growthRate: 4.35,
    },
    {
      icon: (
      ...
      value: data?.DashboardStats?.complaintCount ?? 0,
      // growthRate: 2.59,
    },
    {
      icon: (
      ...
      color: "#18BFFF",
      title: "Total Users",
      value: data?.DashboardStats?.userCount ?? 0,
    },
  ];
